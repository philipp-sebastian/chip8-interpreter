        ...
        case 0xF:
            switch (instructionData->nn) {
                case 0x07:
                    ...
                    SDL_Log("Executed: Set Vx to Delay Timer (LD_VX_DT)");
                    break;
                case 0x0A:
                    ld_wait_for_key_press(appData->pChip8, instructionData);
                    SDL_Log("Executed: Wait for Key Press (LD_KEY)");
                    break;
                case 0x15:
                    ld_set_delay_timer(appData->pChip8, instructionData);
                    SDL_Log("Executed: Set Delay Timer (LD_DT)");
                    break;
                case 0x18:
                    ld_set_sound_timer(appData->pChip8, instructionData);
                    SDL_Log("Executed: Set Sound Timer (LD_ST)");
                    break;
                case 0x1E:
                    add_add_i_and_vx(appData->pChip8, instructionData);
                    SDL_Log("Executed: Add Vx to I (ADD_I_VX)");
                    break;
                case 0x29:
                    ld_set_i_to_sprite_addr(appData->pChip8, instructionData);
                    SDL_Log("Executed: Set I to Sprite Address (LD_I_SPRITE)");
                    break;
                case 0x33:
                    ld_store_bcd(appData->pChip8, instructionData);
                    SDL_Log("Executed: Store BCD Representation (LD_BCD)");
                    break;
                case 0x55:
                    ld_store_v0_to_vx(appData->pChip8, instructionData);
                    SDL_Log("Executed: Store Registers V0 to Vx (LD_STORE_VX)");
                    break;
                case 0x65:
                    ld_read_v0_to_vx(appData->pChip8, instructionData);
                    SDL_Log("Executed: Read Registers V0 to Vx (LD_READ_VX)");
                    break;
                default:
                    SDL_Log("Error: Invalid Fx Command");
                    break;
            }
            break;
        default:
            SDL_Log("Error: Invalid Command");
            break;
    }
